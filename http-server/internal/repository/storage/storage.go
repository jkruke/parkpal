...
func (ms *memStore) GetBikeByLicensePlate(ctx context.Context, plate string) (*entity.Bike, error) {
	...
	query := "SELECT lp.id, lp.name, pl.id, pl.name, pl.latitude, pl.longitude, pl.totalSpace, pl.congestionRate FROM license_plates lp JOIN parking_lots pl ON pl.id = lp.parkinglot_id WHERE lp.name = ?"
